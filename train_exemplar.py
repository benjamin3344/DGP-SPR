...
from cryodrgn.model2 import HetOnlyVAE, VAEvampprior, VAEexemplar
...
def add_args(parser):
    ...
    parser.add_argument('--prior', type=str, default='vampprior', metavar='P',
                        help='prior: standard, vampprior, exemplar')
    # vampprior
    group = parser.add_argument_group('Vampprior')
    group.add_argument('--number-components', type=int, default=50, metavar='NC',
                        help='number of pseudo-inputs')
    group.add_argument('--pseudoinputs-mean', type=float, default=-0.05, metavar='PM',
                        help='mean for init pseudo-inputs')
    group.add_argument('--pseudoinputs-std', type=float, default=0.01, metavar='PS',
                        help='std for init pseudo-inputs')
    group.add_argument('--use_training_data_init', action='store_true', default=False,
                        help='initialize pseudo-inputs with randomly chosen training data')

    # exemplar
    group = parser.add_argument_group('Exemplar')
    group.add_argument('--approximate-prior', action='store_true', default=False)
    group.add_argument('--approximate-k', type=int, default=10)
    group.add_argument('--number-cachecomponents', type=int, default=10000, metavar='NC',
                        help='number of pseudo-inputs for exemplar')

    return parser


def train_batch2(model, data, lattice, y, ind, posetracker, optim, beta, cache, beta_control=None, ctf_params=None, yr=None,
    ...
    if tran is not None:
        y = preprocess_input(y, lattice, tran)
    if scaler is not None:
        with torch.cuda.amp.autocast():
            cache, z_mu, z_logvar, z, log_p_z, y_recon, mask = run_batch2(model, lattice, data, posetracker, ctf_params, y,
                                                                   ind, cache, yr)
    ...
    if use_amp:
        ...
        loss.backward()
        optim.step()
    return cache, loss.item(), gen_loss.item(), kld.item()
...
def run_batch2(model, lattice, data, posetracker, ctf_params, y, ind, cache, yr=None):
    ...
    exemplars_embedding = model.get_exemplar_set(z_mu, z_logvar, cache, ind)

    cache, exemplars_embedding = exemplars_embeddings_update2(cache, model, lattice, data, exemplars_embedding, device,
                                                       posetracker, ctf_params=ctf_params, use_real=args.use_real,
                                                       prior=True)

    log_p_z = _unparallelize(model).log_p_z(z=(z, ind), exemplars_embedding=exemplars_embedding)
    ...
    return cache, z_mu, z_logvar, z, log_p_z, y_recon, mask
...
def exemplars_embeddings_update2(cache, model, lattice, data, exemplars_embedding, device,
                                posetracker, ctf_params=None, use_real=False, prior=True):

    exemplars_z, log_variance, exemplars_indices = exemplars_embedding
    y = torch.tensor(np.array([data[i][0] for i in exemplars_indices])).to(device)
    ind = exemplars_indices
    rot, trans = posetracker.get_pose(ind)
    ctf_param = ctf_params[ind] if ctf_params is not None else None
    B = len(ind)
    D = lattice.D
    ...
    exemplars_z, log_variance = _unparallelize(model).q_z(*input_, prior=True)
    ...
    cached_log_variance[exemplars_indices] = log_variance
    cache = (cached_z, cached_log_variance)

    return cache, exemplar_set
...
def cached_z_eval(model, lattice, data, batch_size, device, trans=None, ctf_params=None, use_real=False, prior=True):
    # assert not model.training
    cached_z = []
    cached_log_var = []
    data_generator = DataLoader(data, batch_size=batch_size, shuffle=False)
    for minibatch in data_generator:
        ...
        D = lattice.D
        if ctf_params is not None:
            freqs = lattice.freqs2d.unsqueeze(0).expand(B, *lattice.freqs2d.shape) / ctf_params[ind, 0].view(B, 1,
                                                                                                             1)
            c = ctf.compute_ctf(freqs, *torch.split(ctf_params[ind, 1:], 1, 1)).view(B, D, D)
        if trans is not None:
            y = lattice.translate_ht(y.view(B, -1), trans[ind].unsqueeze(1)).view(B, D, D)
        ...
        if ctf_params is not None:
            assert not use_real, "Not implemented"
            input_ = (x * c.sign() for x in input_)  # phase flip by the ctf
        exemplars_embedding, log_variance_z = _unparallelize(model).q_z(*input_, prior=True)
    ...
    cached_z = cached_z.to(device)
    cached_log_var = cached_log_var.to(device)
...
#             input_ = (x * c.sign() for x in input_)  # phase flip by the ctf
#         exemplars_embedding, log_variance_z = _unparallelize(model).q_z(*input_, prior=True)
#         cached_z.append(exemplars_embedding.detach().cpu().numpy())
#         cached_log_var.append(log_variance_z.detach().cpu().numpy())
#     cached_z = np.vstack(cached_z)
#     cached_log_var = np.vstack(cached_log_var)
#     cached_z = cached_z.to(device)
#     cached_log_var = cached_log_var.to(device)
#     return cached_z, cached_log_var
...
def main(args):
    ...
    if args.prior == 'exemplar':
        model = VAEexemplar(lattice, args.qlayers, args.qdim, args.players, args.pdim,
    ...
    logging.info('VAE_Exemplar: param size = %fM ', utils.count_parameters_in_M(model))
...
#____--------------------------------------
        ...
        if args.approximate_prior is True:
            ...
            with torch.no_grad():
                if scaler is not None:
                    with torch.cuda.amp.autocast():
                        cached_z, cached_log_var = cached_z_eval(model, lattice, data, args.batch_size, device, posetracker.trans,
                                                ctf_params, args.use_real, prior=True)
                ...
                if args.checkpoint and epoch % args.checkpoint == 0:
                    ...
                    save_checkpoint(model, optim, epoch, cached_z, cached_log_var, out_weights, out_z)
        ...
        for minibatch in train_queue:
            ...
            cache, loss, gen_loss, kld = train_batch2(model, data, lattice, y, ind, posetracker, optim, beta, cache, args.beta_control,
                                              ctf_params=ctf_params, yr=yr, use_amp=args.amp, scaler=scaler)
            # old training
            # rot, tran = posetracker.get_pose(ind)
            # ctf_param = ctf_params[ind] if ctf_params is not None else None
            # loss, gen_loss, kld = train_batch(model, data, lattice, (y, ind), rot, tran, optim, beta, cache, args.beta_control,
